...
lemma Composition.update_comp_same_block
  ...
  · simp +decide only [Function.update, h, ↓reduceDIte]
...
lemma RHS_compatible_deg
  ...
  have h_sum_deg_trans :
    ...
    have h_union :
        Finset.image
          ...
          Finset.univ := by
      ext i
      simp only [Finset.mem_image, Finset.mem_univ, true_and, Sigma.exists, iff_true]
        ...
        have h_partition :
          ...
          have h_exists_a : ∃ a : Fin (c.blocksFun l), c.embedding l a = i := by
            have h_range : i.val - c.sizeUpTo l.val < c.blocksFun l := by
              ...
              convert hl₁ using 1
              simp +decide only [Fin.is_lt, Composition.sizeUpTo_succ, Nat.add_left_cancel_iff]
              ring!
            use ⟨i.val - c.sizeUpTo l.val, h_range⟩
            simp +decide only [Fin.ext_iff, Composition.coe_embedding]
            omega
  ...
  have h_sum_blocksFun : ∑ l : Fin c.length, (1 - (c.blocksFun l : ℤ)) = (c.length : ℤ) - n := by
    simp +decide only [Finset.sum_sub_distrib, Finset.sum_const, Finset.card_univ, Fintype.card_fin,
      Int.nsmul_eq_mul, mul_one, sub_right_inj]
    norm_cast
    aesop
